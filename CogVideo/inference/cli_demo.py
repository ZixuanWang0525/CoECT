...
from visual_enhancing import CogVideoXImageToVideoPipeline
...
RESOLUTION_MAP = {
    ...
    "models--thudm--cogvideox1.5-5b-i2v": (768, 1360),
...
def generate_video(
    ...
    if generate_type == "i2v":
        ...
        prior = load_interpolate_prior_if_any(image_or_video_path)
...
def load_interpolate_prior_if_any(image_or_video_path: str) -> "torch.Tensor | None":
    try:
        zt_dir = os.environ.get("ZT_DIR", "")
        print(f"[I2V] ZT_DIR = {zt_dir}")

        image_path = Path(image_or_video_path)

        sample_name = image_path.parent.name
        zt_path = Path(zt_dir) / f"{sample_name}.pt"

        print(f"[I2V] image_or_video_path = {image_or_video_path}")
        print(f"[I2V] sample_name = {sample_name}")
        print(f"[I2V] expected prior = {zt_path}")

        if not zt_path.exists():
            return None

        prior = torch.load(zt_path, map_location="cpu")

        if not isinstance(prior, torch.Tensor):
            return None

        print(f"[I2V] Loaded prior: {zt_path}")
        print(f"[I2V] Raw prior shape: {tuple(prior.shape)}")


        if prior.ndim == 5:
            b, d1, d2, h, w = prior.shape
            prior = prior.permute(0, 2, 1, 3, 4).contiguous()
        else:
            return None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_frames", type=int, default=45, help="Number of steps for the inference process"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional fixed seed. Omit this argument to use random sampling.",
    )
