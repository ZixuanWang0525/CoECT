    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                """
                # —— 在这里加先验（已对齐到 [B,F,C,H,W] 的 prior_prepped）——
                if prior_prepped is not None:
                    prior = prior_prepped
                    # 做 CFG 时要把先验也复制一份
                    if do_classifier_free_guidance:
                        prior = torch.cat([prior] * 2, dim=0)
                    # 批维不一致就 expand
                    if prior.shape[0] != latent_model_input.shape[0]:
                        prior = prior.expand(latent_model_input.shape[0], -1, -1, -1, -1)

                    # 余弦衰减的权重
                    lambda_prior_base = 0.1
                    lambda_prior = lambda_prior_base * 0.5 * (1 + math.cos(math.pi * i / len(timesteps)))

                    latent_model_input = latent_model_input + lambda_prior * prior
                # —— 先验注入到此为止 —— 
                """
